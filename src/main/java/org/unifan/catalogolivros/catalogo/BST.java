...
public class BST {
    ...
    private void atualizarAltura(No no) {
        if (no != null) {
            int altEsq = altura(no.getEsq());
            int altDir = altura(no.getDir());
     ...
     */
    private No inserirRec(No no, Livro livro) {
        ...
        if (livro.getId() > 0) {
            if (livro.getId() < no.getLivro().getId()) {
                no.setEsq(inserirRec(no.getEsq(), livro));
            } else if (livro.getId() > no.getLivro().getId()) {
                no.setDir(inserirRec(no.getDir(), livro));
            } else {
